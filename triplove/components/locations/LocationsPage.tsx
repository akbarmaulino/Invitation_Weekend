...
function detectCategory(r: NominatimResult) {
  ...
  if (
    ["restaurant", "fast_food", "food_court", "eatery", "restoran"].some(
      (k) => typ.includes(k) || cls.includes(k),
    )
  )
  ...
  if (
    ["hotel", "hostel", "guest_house", "motel", "lodging"].some(
      (k) => typ.includes(k) || cls.includes(k),
    )
  )
    return { category: "Menginap", type_key: "hotel", subtype: "Hotel" };
  if (
    ["attraction", "museum", "theme_park", "zoo", "landmark"].some(
      (k) => typ.includes(k) || cls.includes(k),
    )
  )
    return { category: "Wisata", type_key: "wisata", subtype: "Tempat Wisata" };
  if (
    ["shop", "store", "mall", "market"].some(
      (k) => typ.includes(k) || cls.includes(k),
    )
  )
    return {
      category: "Belanja",
      type_key: "belanja",
      subtype: "Toko & Belanja",
    };
  if (
    ["cinema", "theater", "entertainment", "nightclub", "bar"].some(
      (k) => typ.includes(k) || cls.includes(k),
    )
  )
    return { category: "Hiburan", type_key: "hiburan", subtype: "Hiburan" };
  if (
    ["park", "garden", "nature"].some((k) => typ.includes(k) || cls.includes(k))
  )
...
export default function LocationsPage() {
  ...
  >("idle");
  const [hoveredResult, setHoveredResult] = useState<string | number | null>(
    null,
  );
  ...
  useEffect(() => {
    const handleGlobalPaste = (e: ClipboardEvent) => {
      if (photoMode !== "file" || !showAddModal) return;
      const items = Array.from(e.clipboardData?.items || []);
      const imageItem = items.find((item) => item.type.startsWith("image/"));
      if (imageItem) {
        const blob = imageItem.getAsFile();
        if (blob) {
          setNewFile(blob as File);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setSearchError("Gagal mencari. Cek koneksi internet.");
      } finally {
        setSearching(false);
      }
    }, 600);
  ...
  function handleSelectResult(r: NominatimResult) {
    ...
    setForm({
      ...
      maps_url:
        r.lat && r.lon ? `https://www.google.com/maps?q=${r.lat},${r.lon}` : "",
  ...
  async function handleSave() {
    if (!form.name.trim()) {
      setSaveStatus("⚠️ Nama wajib diisi!");
      return;
    }
    if (!form.type_key) {
      setSaveStatus("⚠️ Pilih kategori & sub-tipe!");
      return;
    }
    setSaving(true);
    ...
    if (photoMode === "url" && photoUrl.startsWith("http")) {
      ...
      setSaveStatus("📷 Upload foto...");
      imageUrl = await uploadImage(newFile, "anon");
    }
    const { error } = await supabase.from("trip_ideas_v2").insert([
      {
        idea_name: form.name.trim(),
        type_key: form.type_key,
        day_of_week: "",
        photo_url: imageUrl,
  ...
  return (
    ...
    >
      ...
      <Navbar />
      <div
        style={{ maxWidth: 920, margin: "0 auto", padding: "28px 16px 100px" }}
      >
        {/* Header */}
        <div
          style={{ marginBottom: 28, position: "relative", paddingLeft: 16 }}
        >
          <div
            style={{
              position: "absolute",
          ...
          <h1
            style={{
              ...
              fontWeight: 900,
              color: C.espresso,
              fontSize: "clamp(1.7em,4vw,2.4em)",
              margin: "0 0 6px",
              lineHeight: 1.15,
          ...
          >
            Temukan{" "}
            <em style={{ color: C.mocha, fontStyle: "italic" }}>Tempat Seru</em>
          </h1>
        ...
        <div
          style={{
            ...
            borderRadius: 20,
            border: `1.5px solid ${C.sand}`,
            boxShadow: "0 6px 32px rgba(3,37,76,0.08)",
            padding: "24px",
            marginBottom: 20,
            position: "relative",
            zIndex: 1,
        ...
        >
          ...
          <div
            style={{
              ...
              boxShadow: "0 4px 14px rgba(3,37,76,0.25)",
            ...
            <div className="loc-grid">
              <div className="loc-list">
                {searching && (
                  ...
                  >
                    <div
                      style={{ fontSize: "2em", marginBottom: 8, opacity: 0.4 }}
                    >
                      🗺️
                    </div>
                    Sedang mencari...
                      ...
                      >
                        ...
                        >
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            {det && (
                              <span className="loc-tag">{det.subtype}</span>
                            )}
                          </div>
                        </div>
                        <button
                          ...
                          onMouseEnter={(e) =>
                            (e.currentTarget.style.background = C.mocha)
                          }
                          onMouseLeave={(e) =>
                            (e.currentTarget.style.background = C.espresso)
                          }
                        >
                          + Tambah
                        </button>
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    {selected ? "Dari Pencarian" : "Manual"}
                  </p>
                  <h3
                    style={{
                      margin: "2px 0 0",
              ...
              {autoDetected && (
                ...
                >
                  ...
                  <strong>{autoDetected.subtype}</strong>
                </div>
              )}
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      {[...new Set(categories.map((c) => c.category))].map(
                        (cat) => (
                  ...
                  <div>
                    ...
                    <select
                      style={{ ...mInp, opacity: !form.category ? 0.5 : 1 }}
                      className="loc-modal-inp"
                      value={form.type_key}
                      disabled={!form.category}
                      onChange={(e) => {
                        const cat = categories.find(
                          (c) => c.type_key === e.target.value,
                        );
                        setForm((f) => ({
                          ...f,
                    ...
                    >
                      ...
                      {subtypesForCat.map((s) => (
                        <option key={s.type_key} value={s.type_key}>
                          {s.subtype}
                        </option>
                      ))}
                      <option value="__custom__">➕ Baru...</option>
                ...
                <div>
                  ...
                  <textarea
                    style={
                      { ...mInp, resize: "vertical" } as React.CSSProperties
                    }
                    className="loc-modal-inp"
                    value={form.address}
                    rows={2}
                ...
                <div>
                    ...
                    {(["file", "url"] as const).map((mode) => (
                      <button
                        ...
                        style={{
                          ...
                          fontWeight: 600,
                          cursor: "pointer",
                          border: `1.5px solid ${photoMode === mode ? C.mocha : C.sand}`,
                          background: photoMode === mode ? C.espresso : C.white,
                          color: photoMode === mode ? C.white : C.muted,
                          transition: "all .15s",
                          fontFamily: "inherit",
                        }}
                      >
                  ...
                  {photoMode === "file" ? (
                    <div
                      tabIndex={0}
                      onClick={() =>
                        document.getElementById("loc-photo-input")?.click()
                      }
                      style={{
                    ...
                    >
                      {photoPreview ? (
                        ...
                        <span>
                          📎 Klik untuk upload, atau <strong>Ctrl+V</strong>{" "}
                          paste gambar
                        </span>
                      )}
                      <input
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    width: "100%",
                    background: saving ? C.sand : C.espresso,
                    color: saving ? C.muted : "white",
                    border: "none",
                    fontWeight: 700,
