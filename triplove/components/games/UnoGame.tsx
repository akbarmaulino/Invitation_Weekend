...
// When there's a pending draw stack, you can only counter with draw cards
...
function RulesPanel({ onClose }: { onClose: () => void }) {
  const rules = [
    ...
    { icon: '🚨', title: 'UNO!', desc: 'Saat punya 1 kartu, tekan UNO! Lupa = kena +2 kartu.' },
...
interface GameState {
  ...
  startCards: number
  pendingDraw: number  // stacked draw penalty waiting to be resolved
}
...
function buildInitialState(startCards: number): GameState {
  ...
  return { deck: d, hands: [h0, h1], discard: [top], currentColor: top.color, currentPlayer: 0, direction: 1, winner: null, unoCalledBy: null, startCards, pendingDraw: 0 }
}
...
export default function UnoGame() {
  ...
  // Toggle card selection (multi-card: must share same value/type)
  function toggleSelect(idx: number) {
    ...
    setSelectedCards(prev => {
      ...
      if (prev.length === 0) {
        // First card: must be playable on discard (or counter if pendingDraw)
      ...
      // Subsequent cards: only need same value as first selected card
  ...
  function playSelectedCards() {
    ...
    // selectedCards order = tap order = play order (bottom to top)
    const orderedIdxs = [...selectedCards]
    const orderedCards = orderedIdxs.map(i => myHand[i])

    // First card must be playable (or counter if pendingDraw)
    ...
    // Middle cards just need same value (already enforced in toggleSelect)
    // No further chain validation needed — all cards share same value

    // Last card is wild -> pick color
    ...
    // newColor = color of last played card
  ...
  function applyPlay(cardIdxs: number[], cards: UnoCard[], newColor: Color) {
    ...
    // Remove played cards (sort descending to splice correctly)
    ...
    // Check UNO
    gs.unoCalledBy = gs.hands[myIndex].length === 1 ? myIndex : null
    if (gs.hands[myIndex].length === 0) { gs.winner = myIndex; pushState(gs); setPhase('finished'); return }
    ...
    // Skip / reverse-as-skip
    if (skipCount + (reverseCount % 2 === 1 ? 1 : 0) > 0) {
      nextP = getNextPlayer(nextP, gs.direction)
    }

    if (totalDrawAdded > 0) {
      // Add to pending draw stack — opponent must counter or take all
      gs.pendingDraw = (gs.pendingDraw || 0) + totalDrawAdded
      // Don't give cards yet — wait for opponent's turn resolution
      nextP = getNextPlayer(myIndex, gs.direction)
    }
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[5, 7, 10].map(n => (
                <button key={n} onClick={() => setStartCards(n)} style={{
                  ...
                  background: startCards === n ? '#E8192C' : 'rgba(255,255,255,0.07)',
                  color: 'white', border: startCards === n ? '2px solid #FF6B7A' : '2px solid rgba(255,255,255,0.12)',
                  fontWeight: 800, cursor: 'pointer', fontSize: '1em', fontFamily: 'inherit',
                  boxShadow: startCards === n ? '0 4px 16px rgba(232,25,44,0.4)' : 'none',
  ...
  const oppForgotUno = oppHasOne && gameState.unoCalledBy !== opp
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 50% 0%, #0d2a4a 0%, #060d1a 60%)', fontFamily: '"Nunito", sans-serif', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {myHand.length === 1 && gameState.unoCalledBy !== myIndex && (
                <button onClick={callUno} style={{ padding: '5px 14px', borderRadius: 8, background: '#FFD900', color: '#333', border: 'none', fontWeight: 900, cursor: 'pointer', fontSize: '0.82em', animation: 'pulse 0.8s infinite', fontFamily: 'inherit' }}>🚨 UNO!</button>
              )}
              {myHand.length === 1 && gameState.unoCalledBy === myIndex && (
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', minHeight: 78 }}>
            {myHand.map((card, i) => {
              ...
              const playable = isMyTurn && (
                selectedCards.includes(i) ? true  // already selected = always show active
                : firstSelected ? card.value === firstSelected.value  // subsequent: same value as first
                : gameState.pendingDraw > 0 ? canCounter(card)         // pending draw: only counters
                : canPlay(card, top, gameState.currentColor)            // normal: canPlay
              )
      ...
      {phase === 'finished' && gameState.winner !== null && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60, backdropFilter: 'blur(6px)' }}>
          <div style={{ background: '#0f1729', borderRadius: 28, padding: '40px 32px', maxWidth: 340, width: '90%', textAlign: 'center', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', margin: '0 0 28px', fontSize: '0.9em' }}>
              {gameState.winner === myIndex ? 'Berhasil habiskan kartu duluan!' : 'Coba lagi lain kali!'}
            </p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setPhase('setup'); setGameId(null); setGameState(null); setInviteCode(null); setSelectedCards([]) }} style={{ flex: 1, padding: '13px', borderRadius: 12, background: 'linear-gradient(135deg, #E8192C, #9B0018)', color: 'white', border: 'none', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 16px rgba(232,25,44,0.4)' }}>🔄 Main Lagi</button>
              <Link href="/games" style={{ flex: 1, padding: '13px', borderRadius: 12, background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.12)', fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}>🎮 Games</Link>
            </div>
          </div>
        </div>
      )}
