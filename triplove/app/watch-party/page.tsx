...
const REACTIONS = ["❤️", "😂", "😱", "😭", "🔥", "👏", "😍", "💀"];
const TURN_USER =
  process.env.NEXT_PUBLIC_TURN_USERNAME || "52361553299cc352d159aa8a";
const TURN_CRED = process.env.NEXT_PUBLIC_TURN_CREDENTIAL || "UqT3j4VoaECWj4on";
const TURN_DOMAIN =
  process.env.NEXT_PUBLIC_TURN_DOMAIN || "global.relay.metered.ca";
const ICE_SERVERS: RTCIceServer[] = [
  ...
  ...(TURN_USER
    ? [
        {
          urls: `turn:${TURN_DOMAIN}:80`,
          username: TURN_USER,
          credential: TURN_CRED,
        },
        {
          urls: `turn:${TURN_DOMAIN}:80?transport=tcp`,
          username: TURN_USER,
          credential: TURN_CRED,
        },
        {
          urls: `turn:${TURN_DOMAIN}:443`,
          username: TURN_USER,
          credential: TURN_CRED,
        },
        {
...
export default function WatchPartyPage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`watch:${roomCode}`, {
    ...
    ch.on("broadcast", { event: "sync" }, ({ payload }) => {
      ...
      if (s.playing) playStartRef.current = Date.now() - (s.time + lag) * 1000;
    });
  ...
  async function startScreenShare() {
    try {
      ...
      stream
        .getTracks()
        .forEach((t: MediaStreamTrack) => pc.addTrack(t, stream));
  ...
  async function handleWebRTCSignal(payload: any) {
    if (isHost) {
      ...
      if (payload.type === "offer") {
        log(
          "PARTNER: got offer, candidates=" + (payload.candidates?.length ?? 0),
        );
        ...
        log("PARTNER: setRemoteDescription done");
  ...
  async function createRoom() {
    if (!myName.trim() || !filmTitle.trim()) {
      setError("Isi nama dan judul film dulu!");
      return;
    }
    const code = genCode();
    setRoomCode(code);
  ...
  async function joinRoom() {
    if (!myName.trim() || !joinCode.trim()) {
      setError("Isi nama dan kode room dulu!");
      return;
    }
  ...
  function sendReaction(emoji: string) {
    ...
    supabase
      .channel(`watch:${roomCode}`)
      .send({ type: "broadcast", event: "reaction", payload: r });
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          {tab === "create" ? (
            <Input
              label="Judul film / tontonan"
              value={filmTitle}
              onChange={setFilmTitle}
          ...
          <button
            ...
            style={{
              padding: "14px",
              background: `linear-gradient(135deg, ${GOLD} 0%, #a07840 100%)`,
              border: "none",
              borderRadius: 12,
              color: DARK,
              fontWeight: 800,
      ...
      >
        Sync timer + live chat + screen share (YouTube & file lokal). Netflix
        tidak support screen capture.
      </p>
    </div>
  );
}
    ...
    >
      ...
      <h2
        style={{
          fontFamily: "'Playfair Display', serif",
          color: CREAM,
          fontSize: "clamp(1.3em, 4vw, 1.8em)",
          margin: "0 0 6px",
          fontWeight: 400,
        }}
      >
      ...
      >
        ...
        <button
          ...
          style={{
            padding: "8px 20px",
            background: copied ? "rgba(201,169,110,0.2)" : "transparent",
      ...
      >
        <div
          style={{
            ...
            height: 10,
            borderRadius: "50%",
            background: partnerOnline ? "#4ade80" : "#6b7280",
            boxShadow: partnerOnline ? "0 0 8px #4ade80" : "none",
            transition: "all .3s",
          }}
        />
        <span
          style={{
            color: partnerOnline ? "#4ade80" : "rgba(255,255,255,0.4)",
            fontSize: "0.82em",
          }}
        >
      ...
      <button
        ...
        style={{
          ...
          border: "none",
          borderRadius: 12,
          color: partnerOnline ? DARK : "rgba(255,255,255,0.3)",
          fontWeight: 800,
          fontSize: "1em",
          ...
          >
            <div
              style={{
                ...
                borderRadius: "50%",
                background: partnerOnline ? "#4ade80" : "#6b7280",
                boxShadow: partnerOnline ? "0 0 6px #4ade80" : "none",
              }}
            />
            <span style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.7em" }}>
          ...
          >
            {showScreen ? (
                ...
                {needsTap && (
                  ...
                  >
                    <div style={{ textAlign: "center" }}>
                      ...
                      >
                        Ketuk layar untuk memutar video
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div style={{ textAlign: "center", padding: 24 }}>
                ...
                >
                  {isHost
                    ? "Mulai screen share di bawah untuk streaming ke pasangan"
                    : "Menunggu host mulai screen share..."}
                </p>
              </div>
            )}
            ...
            {isHost && (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${screenActive ? "#ef4444" : "rgba(255,255,255,0.15)"}`,
                  borderRadius: 8,
                  color: screenActive ? "#ef4444" : "rgba(255,255,255,0.7)",
                  fontSize: "0.75em",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
              ...
              {messages.map((m: ChatMessage) => (
                <div
                  ...
                  style={{
                    display: "flex",
                    flexDirection: "column",
                    alignItems: m.sender === myName ? "flex-end" : "flex-start",
                  }}
                >
                  <span
                    style={{
                    ...
                    style={{
                      background:
                        m.sender === myName
                          ...
                          : "rgba(255,255,255,0.07)",
                      border: `1px solid ${m.sender === myName ? "rgba(201,169,110,0.3)" : "rgba(255,255,255,0.08)"}`,
                      borderRadius:
                        m.sender === myName
                          ? "12px 12px 2px 12px"
                          : "12px 12px 12px 2px",
                  ...
                  >
                    <p
                      style={{
                        margin: 0,
                        color: m.sender === myName ? GOLD : CREAM,
                        fontSize: "0.82em",
                        lineHeight: 1.4,
                      }}
                    >
